...
start_webtest(){
	...
	if [ "$array11" == "1" ] || [ "$array11" == "2" ] || [ "$array11" == "3" ] || [ "$array11" == "5" ];then
		...
		EOF
			...
			then
				my_best_node=$nu
				my_best_node_delay=$result
			fi
            sleep 1

			dbus set my_ssconf_basic_webtest_$nu=$result
...
if [ "$ssconf_basic_test_node" != "0" ];then
	...
	dbus set my_best_node=$my_best_node
	dbus set my_best_node_delay=$my_best_node_delay
fi